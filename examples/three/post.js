...
const getJs = (p) => import(`three/examples/jsm/${p}.js`);
...
(async () => {
	...
	const { RenderPass } = await getJs('postprocessing/RenderPass');
	const { ShaderPass } = await getJs('postprocessing/ShaderPass');
	const { BloomPass } = await getJs('postprocessing/BloomPass');
	const { FilmPass } = await getJs('postprocessing/FilmPass');
	const { DotScreenPass } = await getJs('postprocessing/DotScreenPass');
	const { MaskPass, ClearMaskPass } = await getJs('postprocessing/MaskPass');
	const { EffectComposer } = await getJs('postprocessing/EffectComposer');
	const { TexturePass } = await getJs('postprocessing/TexturePass');
	const { BleachBypassShader } = await getJs('shaders/BleachBypassShader');
	const { ColorifyShader } = await getJs('shaders/ColorifyShader');
	const { HorizontalBlurShader } = await getJs('shaders/HorizontalBlurShader');
	const { VerticalBlurShader } = await getJs('shaders/VerticalBlurShader');
	const { SepiaShader } = await getJs('shaders/SepiaShader');
	const { VignetteShader } = await getJs('shaders/VignetteShader');
	const { GLTFLoader } = await getJs('loaders/GLTFLoader');
